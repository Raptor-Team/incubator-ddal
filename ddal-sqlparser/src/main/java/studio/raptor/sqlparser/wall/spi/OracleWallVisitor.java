...
import studio.raptor.sqlparser.dialect.oracle.visitor.OracleASTVisitorAdapter;
...
public class OracleWallVisitor extends OracleASTVisitorAdapter implements WallVisitor {
  ...
  public boolean visit(OracleMultiInsertStatement x) {
    if (!config.isInsertAllow()) {
      this.getViolations()
          .add(new IllegalSQLObjectViolation(ErrorCode.INSERT_NOT_ALLOW, "insert not allow",
              this.toSQL(x)));
      return false;
    }
    WallVisitorUtils.initWallTopStatementContext();
