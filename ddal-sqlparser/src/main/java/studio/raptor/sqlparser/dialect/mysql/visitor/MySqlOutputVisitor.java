...
public class MySqlOutputVisitor extends SQLASTOutputVisitor implements MySqlASTVisitor {

  {
    ...
    // unsupported default modified by Sam
  ...
  public boolean visit(SQLMethodInvokeExpr x) {
    ...
    if (("CONVERT".equalsIgnoreCase(x.getMethodName())) || "CHAR"
        .equalsIgnoreCase(x.getMethodName())) {
      if (x.getOwner() != null) {
        x.getOwner().accept(this);
        print('.');
      }
  ...
  public boolean visit(MySqlSubPartitionByList x) {
    ...
    if (x.getExpr() != null) {
      ...
      if (x.getColumns().size() == 1 && Boolean.TRUE
          .equals(x.getAttribute("ads.subPartitionList"))) {
        print('(');
      } else {
        print0(ucase ? "COLUMNS (" : "columns (");
      }
  ...
  public boolean visit(MySqlDeclareHandlerStatement x) {
    ...
    print0(ucase ? "DECLARE " : "declare ");
    print0(ucase ? x.getHandleType().toString().toUpperCase()
        : x.getHandleType().toString().toLowerCase());
