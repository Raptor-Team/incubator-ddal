 ...
 * @since 3.0.0
 */
public class CountAggregationTest extends AutoPrepareTestingEnv {

    /**
     * 单分片Count
     */
    @Test
    public void testCountWithGlobalTable() {
     ...
     */
    @Test
    public void testCountWithSingleShard() {
     ...
     */
    @Test
    public void testCountWithMultiShard() {
        String sql = "select count(*) from teacher";
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    Assert.assertEquals(16, resultSet.getInt(1));
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
