...
public final class ParseUtil {
    ...
    public static String trimSqlHint(String sql) {
        StringBuilder commentBuilder = new StringBuilder();
        int firstCharIndex = sql.indexOf("/*!hint", 0);
        if (-1 != firstCharIndex) {
            firstCharIndex += 6;
        }
        char ch;
        for (int i = firstCharIndex + 1, commentLen = sql.length(); i < commentLen; i++) {
            if ((ch = sql.charAt(i)) == '*' && (ch = sql.charAt(i + 1)) == '/') {
                break;
            }
