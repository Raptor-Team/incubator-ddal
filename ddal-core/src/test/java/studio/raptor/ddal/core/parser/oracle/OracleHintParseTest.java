 ...
 * @since 3.0.0
 */
public class OracleHintParseTest {
...
//
//        parseEngine = SQLParserFactory.create(databaseType, sql, Collections.emptyList(), ShardConfig.getInstance().getVirtualDb("crmdb"));
//        try {
//            parseEngine.parse();
//        } catch (RuntimeException e) {
//            e.printStackTrace();
...
//        Assert.assertThat(parsedResult.getRouteContext().getHintContext().getHiddenShardValuePairs().size(), Is.is(1));
//        Assert.assertThat(parsedResult.getRouteContext().getHintContext().getHiddenShardValuePairs().containsKey("customer_id"), Is.is(true));
//        Assert.assertThat(String.valueOf(parsedResult.getRouteContext().getHintContext().getHiddenShardValuePairs().get("customer_id")), Is.is("8920021"));
//    }
}
