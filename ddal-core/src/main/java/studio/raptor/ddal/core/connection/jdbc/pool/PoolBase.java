...
import static studio.raptor.ddal.core.connection.jdbc.pool.ProxyConnection.DIRTY_BIT_AUTOCOMMIT;
import static studio.raptor.ddal.core.connection.jdbc.pool.ProxyConnection.DIRTY_BIT_CATALOG;
import static studio.raptor.ddal.core.connection.jdbc.pool.ProxyConnection.DIRTY_BIT_ISOLATION;
import static studio.raptor.ddal.core.connection.jdbc.pool.ProxyConnection.DIRTY_BIT_NETTIMEOUT;
import static studio.raptor.ddal.core.connection.jdbc.pool.ProxyConnection.DIRTY_BIT_READONLY;
import static studio.raptor.ddal.core.connection.jdbc.util.UtilityElf.createInstance;
...
import studio.raptor.ddal.core.connection.jdbc.util.DriverDataSource;
import studio.raptor.ddal.core.connection.jdbc.util.PropertyElf;
import studio.raptor.ddal.core.connection.jdbc.util.UtilityElf;
import studio.raptor.ddal.core.connection.jdbc.util.UtilityElf.DefaultThreadFactory;

abstract class PoolBase {

  private static final String[] RESET_STATES = {"readOnly", "autoCommit", "isolation", "catalog",
      "netTimeout"};
  ...
  PoolBase(final HikariConfig config) {
    ...
    this.transactionIsolation = UtilityElf
        .getTransactionIsolation(config.getTransactionIsolation());
    ...
    try {
      try {
        if (isUseJdbc4Validation) {
          return connection.isValid((int) MILLISECONDS.toSeconds(
        ...
        try (Statement statement = connection.createStatement()) {
          if (isNetworkTimeoutSupported != TRUE) {
            setQueryTimeout(statement, (int) MILLISECONDS.toSeconds(
                Math.max(1000L, validationTimeout)));
          }
      ...
      LOGGER
          .warn("{} - Failed to validate connection {} ({})", poolName, connection, e.getMessage());
      return false;
    }
  }
  ...
  void resetConnectionState(final Connection connection, final ProxyConnection proxyConnection,
      final int dirtyBits) throws SQLException {
    int resetBits = 0;
    ...
    if ((dirtyBits & DIRTY_BIT_CATALOG) != 0 && catalog != null && !catalog
        .equals(proxyConnection.getCatalogState())) {
    ...
    if (resetBits != 0 && LOGGER.isDebugEnabled()) {
      LOGGER.debug("{} - Reset ({}) on connection {}", poolName, stringFromResetBits(resetBits),
          connection);
    }
  }
  ...
  void registerMBeans(final HikariPool hikariPool) {
    ...
    try {
      ...
      final ObjectName beanConfigName = new ObjectName(
          "com.zaxxer.hikari:type=PoolConfig (" + poolName + ")");
      final ObjectName beanPoolName = new ObjectName(
  ...
  private void initializeDataSource() {
    ...
    if (dsClassName != null && dataSource == null) {
      ...
      dataSource = new DriverDataSource(jdbcUrl, driverClassName, dataSourceProperties, username,
          password);
    }
  ...
  Connection newConnection() throws Exception {
    ...
    try {
      ...
      connection = (username == null) ? dataSource.getConnection()
  ...
  private void checkDriverSupport(final Connection connection) throws SQLException {
    if (!isValidChecked) {
      try {
        ...
        LOGGER.error("{} - Failed to execute" + (isUseJdbc4Validation
                ? " isValid() for connection, configure" : "") + " connection test query. ({})",
    ...
    if (isQueryTimeoutSupported != FALSE) {
      try {
        ...
        if (isQueryTimeoutSupported == UNINITIALIZED) {
          ...
          LOGGER.info("{} - Failed to set query timeout for statement. ({})", poolName,
              e.getMessage());
        }
      }
    }
   ...
   */
  private void setNetworkTimeout(final Connection connection, final long timeoutMs)
      throws SQLException {
    if (isNetworkTimeoutSupported == TRUE) {
      connection.setNetworkTimeout(netTimeoutExecutor, (int) timeoutMs);
    }
   ...
   */
  private void executeSql(final Connection connection, final String sql, final boolean isCommit)
      throws SQLException {
    if (sql != null) {
      try (Statement statement = connection.createStatement()) {
  ...
  private void setLoginTimeout(final DataSource dataSource) {
    if (connectionTimeout != Integer.MAX_VALUE) {
      try {
        dataSource
            .setLoginTimeout(Math.max(1, (int) MILLISECONDS.toSeconds(500L + connectionTimeout)));
      } catch (Throwable e) {
        LOGGER.info("{} - Failed to set login timeout for data source. ({})", poolName,
            e.getMessage());
      }
    }
  }
