...
import static studio.raptor.ddal.core.connection.jdbc.util.UtilityElf.getNullIfEmpty;
...
public class HikariConfig implements HikariConfigMXBean {
  ...
  public void setDriverClassName(String driverClassName) {
    ...
    if (driverClass == null) {
      throw new RuntimeException("Failed to load class of driverClassName [" + driverClassName
   ...
   *
   * @param initializationFailTimeout the number of milliseconds before the pool initialization
   * fails, or 0 to validate connection setup but continue with pool start, or less than zero to
    ...
    if (metricRegistry != null) {
      ...
      if (!(metricRegistry instanceof MetricRegistry)) {
        throw new IllegalArgumentException(
            "Class must be an instance of com.codahale.metrics.MetricRegistry");
      }
    }
   ...
   *
   * @param threadFactory the thread factory (setting to null causes the default thread factory to
   * be used)
   */
  ...
  private void validateNumerics() {
    if (maxLifetime != 0 && maxLifetime < SECONDS.toMillis(30)) {
      LOGGER.warn("{} - maxLifetime is less than 30000ms, setting to default {}ms.", poolName,
          MAX_LIFETIME);
      maxLifetime = MAX_LIFETIME;
    }
    ...
    if (leakDetectionThreshold > 0 && !unitTest) {
      if (leakDetectionThreshold < SECONDS.toMillis(2) || (leakDetectionThreshold > maxLifetime
          && maxLifetime > 0)) {
        LOGGER.warn(
    ...
    if (connectionTimeout < 250) {
      LOGGER.warn("{} - connectionTimeout is less than 250ms, setting to {}ms.", poolName,
          CONNECTION_TIMEOUT);
      connectionTimeout = CONNECTION_TIMEOUT;
    }

    if (validationTimeout < 250) {
      LOGGER.warn("{} - validationTimeout is less than 250ms, setting to {}ms.", poolName,
          VALIDATION_TIMEOUT);
      validationTimeout = VALIDATION_TIMEOUT;
    }
  ...
  private void logConfiguration() {
    ...
    final Set<String> propertyNames = new TreeSet<>(
        PropertyElf.getPropertyNames(HikariConfig.class));
    for (String prop : propertyNames) {
      try {
        ...
        if ("initializationFailTimeout".equals(prop)
  ...
  private int generatePoolNumber() {
    ...
    synchronized (System.getProperties()) {
      final int next = Integer.getInteger("studio.raptor.ddal.core.connection.pool_number", 0) + 1;
      System.setProperty("studio.raptor.ddal.core.connection.pool_number", String.valueOf(next));
      return next;
    }
  }
