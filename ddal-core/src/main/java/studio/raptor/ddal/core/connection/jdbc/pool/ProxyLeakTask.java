...
class ProxyLeakTask implements Runnable {
  ...
  private ProxyLeakTask(final ProxyLeakTask parent, final PoolEntry poolEntry) {
    ...
    scheduledFuture = parent.executorService
        .schedule(this, parent.leakDetectionThreshold, TimeUnit.MILLISECONDS);
  }
  ...
  public void run() {
    ...
    LOGGER.warn("Connection leak detection triggered for {}, stack trace follows", connectionName,
        exception);
  }
  ...
  void cancel() {
    ...
    if (isLeaked) {
      LOGGER.info("Previously reported leaked connection {} was returned to the pool (unleaked)",
          connectionName);
    }
  }
}
