...
public class OutputRewrittenSql extends ProcessNode {
  ...
  protected void execute(ProcessContext context) {
    ...
    for (RouteNode routeNode : context.getRouteResult().getRouteNodes()) {
      String finalSql = sql;
      for (Map.Entry<String, String> rewriteAttribute : routeNode.getRewriteAttributes()
          .entrySet()) {
        finalSql = finalSql.replaceAll(rewriteAttribute.getKey(), rewriteAttribute.getValue());
      }
      routeNode.setFinalSql(finalSql);
    }
    if(log.isDebugEnabled()) {
      log.debug("Rewrite result: {}", context.getRouteResult().getRouteNodes());
    }
  }
}
